...
//
// Outlook desktop (Word rendering engine) ignores max-width on <body>/<div>,
// so the brief is wrapped in a centered table with width="720" — that's the
// only width hint Outlook reliably honors. table-layout: fixed + word-wrap
// keep long quotes / URLs from forcing horizontal overflow.
const STYLE = `
  body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif;
         margin: 0; padding: 0; color: #111; line-height: 1.55;
         word-wrap: break-word; overflow-wrap: break-word; }
  table.wrapper { width: 100%; border-collapse: collapse; }
  table.content { width: 100%; max-width: 720px; border-collapse: collapse; }
  td.content-cell { padding: 24px 16px; }
  h1 { font-size: 22px; margin: 0 0 4px; }
  .date { color: #666; font-size: 13px; margin-bottom: 24px; }
  table.item { width: 100%; table-layout: fixed; border-collapse: collapse; border-top: 1px solid #eee; }
...
<title>Podcast Intel — ${dateStr}</title>
<style>${STYLE}</style>
</head>
<body>
  <table class="wrapper" role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td align="center">
      <table class="content" role="presentation" width="720" cellpadding="0" cellspacing="0" border="0">
